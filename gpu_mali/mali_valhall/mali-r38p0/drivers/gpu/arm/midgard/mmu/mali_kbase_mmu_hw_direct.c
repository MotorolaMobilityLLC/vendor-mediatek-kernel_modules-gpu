...
/**
 * kbase_mmu_command_instr - Record an MMU command for instrumentation purposes.
 *
 ...
 */
static void kbase_mmu_command_instr(struct kbase_device *kbdev, u32 kctx_id, u32 cmd, u64 lock_addr,
 ...
 * is issued.
 */
static int kbase_mmu_hw_set_lock_addr(struct kbase_device *kbdev, int as_nr, u64 vpfn, u32 nr,
				      u64 *lock_addr, u64 flush_skip_levels)
{
	int ret;

	ret = lock_region(&kbdev->gpu_props, vpfn, nr, lock_addr);
...
/**
 * kbase_mmu_hw_do_lock_no_wait - Issue LOCK command to the MMU and return without
 *                                waiting for it's completion.
 *
 * @kbdev:             Kbase device to issue the MMU operation on.
 * @as:                Address space to issue the MMU operation on.
 * @vpfn:              MMU Virtual Page Frame Number.
 * @nr:                Number of pages.
 * @lock_addr:         Address of memory region locked for this operation.
 * @flush_skip_levels: Page table levels to skip for MMU operation. (Only
 *                     applicable if GPU supports feature)
 *
 * Return: 0 if issuing the command was successful, otherwise an error code.
 */
static int kbase_mmu_hw_do_lock_no_wait(struct kbase_device *kbdev, struct kbase_as *as, u64 vpfn,
					u32 nr, u64 *lock_addr, u64 flush_skip_levels)
{
	int ret;

	ret = kbase_mmu_hw_set_lock_addr(kbdev, as->number, vpfn, nr, lock_addr, flush_skip_levels);
...
int kbase_mmu_hw_do_lock(struct kbase_device *kbdev, struct kbase_as *as, u64 vpfn, u32 nr,
			 u32 kctx_id, enum kbase_caller_mmu_sync_info mmu_sync_info,
			 u64 flush_skip_levels)
{
	...
	ret = kbase_mmu_hw_do_lock_no_wait(kbdev, as, vpfn, nr, &lock_addr, flush_skip_levels);

	if (!ret)
		ret = wait_ready(kbdev, as->number);

	if (!ret)
		kbase_mmu_command_instr(kbdev, kctx_id, AS_COMMAND_LOCK, lock_addr, mmu_sync_info);

	return ret;
}

int kbase_mmu_hw_do_unlock_no_addr(struct kbase_device *kbdev, struct kbase_as *as, u32 kctx_id,
				   enum kbase_caller_mmu_sync_info mmu_sync_info)
{
	int ret = 0;
	...
	if (!ret) {
		...
		kbase_mmu_command_instr(kbdev, kctx_id, AS_COMMAND_UNLOCK, lock_addr,
					mmu_sync_info);
	}
...
int kbase_mmu_hw_do_unlock(struct kbase_device *kbdev, struct kbase_as *as, u64 vpfn, u32 nr,
			   u32 kctx_id, enum kbase_caller_mmu_sync_info mmu_sync_info,
			   u64 flush_skip_levels)
{
	int ret = 0;
	...
	ret = kbase_mmu_hw_set_lock_addr(kbdev, as->number, vpfn, nr, &lock_addr,
					 flush_skip_levels);

	if (!ret)
		ret = kbase_mmu_hw_do_unlock_no_addr(kbdev, as, kctx_id, mmu_sync_info);
...
int kbase_mmu_hw_do_flush(struct kbase_device *kbdev, struct kbase_as *as, u64 vpfn, u32 nr,
			  u32 cmd, u32 kctx_id, enum kbase_caller_mmu_sync_info mmu_sync_info,
			  u64 flush_skip_levels)
{
	int ret;
	u64 lock_addr = 0x0;
	...
	if ((cmd != AS_COMMAND_FLUSH_PT) && (cmd != AS_COMMAND_FLUSH_MEM)) {
	...
	write_cmd(kbdev, as->number, cmd);

	/* Wait for the command to complete */
	ret = wait_ready(kbdev, as->number);

	if (!ret)
		kbase_mmu_command_instr(kbdev, kctx_id, cmd, lock_addr, mmu_sync_info);
