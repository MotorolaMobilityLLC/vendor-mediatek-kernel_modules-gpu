...
/**
 * mmu_flush_invalidate_as_on_gpu_ctrl() - Perform GPU cache flush operation through GPU_CONTROL
 * interface.
 * @kbdev:         Kbase device to issue the MMU operation on.
 * @as:            Address space to issue the MMU operation on.
 * @flush_info:    Parameters for the flush operation.
 *
 ...
 * The function first issues LOCK to MMU-AS, then issues cache-flush with
 * kbase_gpu_cache_flush_and_busy_wait() function thereafter issues UNLOCK to MMU-AS.
 *
 * Return: Zero if the operation was successful, non-zero otherwise.
 */
static int mmu_flush_invalidate_as_on_gpu_ctrl(struct kbase_device *kbdev, struct kbase_as *as,
					       const struct kbase_mmu_flush_param *flush_info)
{
	u32 gpu_cmd;
	int ret, ret2;

	if (WARN_ON(kbdev == NULL) || WARN_ON(as == NULL) || WARN_ON(flush_info == NULL))
		return -EINVAL;
	...
	if (flush_info->op == KBASE_MMU_OP_FLUSH_PT) {
		gpu_cmd = GPU_COMMAND_CACHE_CLN_INV_L2;
	} else if (flush_info->op == KBASE_MMU_OP_FLUSH_MEM) {
		gpu_cmd = GPU_COMMAND_CACHE_CLN_INV_L2_LSC;
	} else {
	...
	ret = kbase_mmu_hw_do_lock(kbdev, as, flush_info->vpfn, flush_info->nr, flush_info->kctx_id,
				   flush_info->mmu_sync_info, flush_info->flush_skip_levels);
	if (ret)
		return ret;
	...
	ret2 = kbase_mmu_hw_do_unlock_no_addr(kbdev, as, flush_info->kctx_id,
					      flush_info->mmu_sync_info);
 ...
 * @flush_info: Non-NULL pointer to struct containing information about the MMU
					     ...
					     const struct kbase_mmu_flush_param *flush_info)
{
	...
	if (kbdev->pm.backend.gpu_powered && (!kctx || kctx->as_nr >= 0)) {
		...
		err = mmu_flush_invalidate_as_on_gpu_ctrl(kbdev, &kbdev->as[as_nr], flush_info);
	}
 ...
 *              operation to perform.
 *
 * This function performs the cache flush operation described by @flush_info.
						   ...
						   u32 kctx_id)
{
	...
	u64 no_skips = pgd_level_to_skip_flush(0xF);
	...
	if (mmu_flush_cache_on_gpu_ctrl(kbdev)) {
		...
		const struct kbase_mmu_flush_param flush_info = {
			...
			.flush_skip_levels = no_skips,
		};

		spin_lock_irqsave(&kbdev->hwaccess_lock, irq_flags);
		err = mmu_flush_invalidate_as_on_gpu_ctrl(kbdev, faulting_as, &flush_info);
		spin_unlock_irqrestore(&kbdev->hwaccess_lock, irq_flags);
	} else {
		mmu_hw_operation_begin(kbdev);
		err = kbase_mmu_hw_do_flush(kbdev, faulting_as, start_pfn, nr, AS_COMMAND_FLUSH_PT,
					    kctx_id, mmu_sync_info, no_skips);
			...
			struct kbase_as *faulting_as)
{
	...
	ret = kbase_mmu_update_pages_no_flush(kctx, fault_pfn, fault_phys_addr, 1, region->flags,
					      region->gpu_alloc->group_id, NULL);
...
page_fault_retry:
	...
	if (fault_rel_pfn < current_backed_size) {
		...
		mmu_hw_operation_begin(kbdev);
		if (!mmu_flush_cache_on_gpu_ctrl(kbdev))
			err = kbase_mmu_hw_do_unlock_no_addr(kbdev, faulting_as, kctx->id,
							     mmu_sync_info);
		else {
			/* Value used to prevent skipping of any levels when flushing */
			u64 no_skips = pgd_level_to_skip_flush(0xF);
			/* Set lock address to make use of fine-grain control of MMU and
			 * avoid invalidating all caches associated with the faulting
			 * address space (including TLBs).
			 */
			err = kbase_mmu_hw_do_unlock(kbdev, faulting_as, fault_pfn, 1, kctx->id,
						     mmu_sync_info, no_skips);
		}
		mmu_hw_operation_end(kbdev);
	...
	if (grown) {
		...
		if (mmu_flush_cache_on_gpu_ctrl(kbdev)) {
			...
			mmu_hw_operation_begin(kbdev);
			err = kbase_mmu_hw_do_flush(kbdev, faulting_as, fault->addr >> PAGE_SHIFT,
						    new_pages, AS_COMMAND_FLUSH_PT, kctx->id,
						    mmu_sync_info,
				 ...
				 enum kbase_caller_mmu_sync_info mmu_sync_info)
{
	...
	struct kbase_device *kbdev;
	enum kbase_mmu_op_type flush_op;
	struct kbase_mmu_flush_param flush_info;
	u64 dirty_pgds = 0;
	...
	flush_info = (struct kbase_mmu_flush_param){
		.vpfn = vpfn,
		.nr = nr,
		.op = KBASE_MMU_OP_FLUSH_PT,
		.kctx_id = kctx->id,
	...
	flush_info.flush_skip_levels = pgd_level_to_skip_flush(dirty_pgds);
	 ...
	 */
	if (mmu_flush_cache_on_gpu_ctrl(kbdev))
		mmu_invalidate(kbdev, kctx, kctx->as_nr, &flush_info);
	else
		mmu_flush_invalidate(kbdev, kctx, kctx->as_nr, &flush_info);
	return 0;
...
fail_unlock:
	...
	flush_info.flush_skip_levels = pgd_level_to_skip_flush(dirty_pgds);
	if (mmu_flush_cache_on_gpu_ctrl(kbdev))
		mmu_flush_invalidate_on_gpu_ctrl(kbdev, kctx, kctx->as_nr, &flush_info);
	else
		mmu_flush_invalidate(kbdev, kctx, kctx->as_nr, &flush_info);
	return err;
}
...
{
	int err;
	struct kbase_mmu_flush_param flush_info = { 0 };
	u64 dirty_pgds = 0;
	...
	flush_info.vpfn = vpfn;
	flush_info.nr = nr;
	flush_info.op = KBASE_MMU_OP_FLUSH_PT;
	flush_info.mmu_sync_info = mmu_sync_info;
	flush_info.kctx_id = mmut->kctx ? mmut->kctx->id : 0xFFFFFFFF;
	flush_info.flush_skip_levels = pgd_level_to_skip_flush(dirty_pgds);
	 ...
	 */
	if (mmu_flush_cache_on_gpu_ctrl(kbdev))
		mmu_invalidate(kbdev, mmut->kctx, as_nr, &flush_info);
	else
		mmu_flush_invalidate(kbdev, mmut->kctx, as_nr, &flush_info);
...
static void kbase_mmu_flush_noretain(struct kbase_context *kctx, u64 vpfn, size_t nr)
{
	...
	if (mmu_flush_cache_on_gpu_ctrl(kbdev)) {
		...
		err = mmu_flush_invalidate_as_on_gpu_ctrl(kbdev, &kbdev->as[kctx->as_nr],
							  &flush_info);
	} else {
		err = kbase_mmu_hw_do_flush(kbdev, &kbdev->as[kctx->as_nr], vpfn, nr,
					    AS_COMMAND_FLUSH_MEM, kctx->id, mmu_sync_info,
					    no_skips);
	}
			   ...
			   const struct kbase_mmu_flush_param *flush_info)
{
	...
	if (kbdev->pm.backend.gpu_powered && (!kctx || kctx->as_nr >= 0)) {
		as_nr = kctx ? kctx->as_nr : as_nr;
		err = kbase_mmu_hw_do_unlock(kbdev, &kbdev->as[as_nr], flush_info->vpfn,
					     flush_info->nr, flush_info->kctx_id,
					     flush_info->mmu_sync_info,
					     flush_info->flush_skip_levels);
	}
				    ...
				    const struct kbase_mmu_flush_param *flush_info)
{
	 ...
	 */
	if (flush_info->op == KBASE_MMU_OP_FLUSH_PT || flush_info->op == KBASE_MMU_OP_FLUSH_MEM) {
		flush_cmd = flush_info->op == KBASE_MMU_OP_FLUSH_PT ? AS_COMMAND_FLUSH_PT :
	...
	mmu_hw_operation_begin(kbdev);
	err = kbase_mmu_hw_do_flush(kbdev, as, flush_info->vpfn, flush_info->nr, flush_cmd,
				    flush_info->kctx_id, flush_info->mmu_sync_info,
				    flush_info->flush_skip_levels);
	mmu_hw_operation_end(kbdev);
				 ...
				 const struct kbase_mmu_flush_param *flush_info)
{
	 ...
	 */
	if (!kctx) {
		mmu_flush_invalidate_as(kbdev, &kbdev->as[as_nr], flush_info);
	} else {
#if !MALI_USE_CSF
		...
		if (ctx_is_in_runpool) {
			...
			mmu_flush_invalidate_as(kbdev, &kbdev->as[kctx->as_nr], flush_info);
 ...
 * @flush_info:  Non-NULL pointer to struct containing information about the flush
 *               operation to perform.
 *
						...
						struct kbase_mmu_flush_param *flush_info)
{
	...
	if (!mmu_flush_cache_on_gpu_ctrl(kbdev)) {
		mmu_flush_invalidate(kbdev, kctx, as_nr, flush_info);
		return;
	} else if (flush_info->op == KBASE_MMU_OP_FLUSH_MEM) {
		mmu_flush_invalidate_on_gpu_ctrl(kbdev, kctx, as_nr, flush_info);
		return;
	}
			     ...
			     struct tagged_addr *phys, size_t nr, int as_nr)
{
	 ...
	 */
	if (phys && nr <= KBASE_PA_RANGE_THRESHOLD_NR_PAGES)
		flush_op = KBASE_MMU_OP_FLUSH_PT;
...
out:
	...
	mmu_flush_invalidate_teardown_pages(kbdev, mmut->kctx, as_nr, phys, &flush_info);
			   ...
			   unsigned long flags, int const group_id)
{
	...
	flush_info = (const struct kbase_mmu_flush_param){
		.vpfn = vpfn,
		.nr = nr,
		.op = KBASE_MMU_OP_FLUSH_MEM,
		.kctx_id = kctx->id,
	...
	if (mmu_flush_cache_on_gpu_ctrl(kctx->kbdev))
		mmu_flush_invalidate_on_gpu_ctrl(kctx->kbdev, kctx, kctx->as_nr, &flush_info);
	else
		mmu_flush_invalidate(kctx->kbdev, kctx, kctx->as_nr, &flush_info);
	return err;
}
