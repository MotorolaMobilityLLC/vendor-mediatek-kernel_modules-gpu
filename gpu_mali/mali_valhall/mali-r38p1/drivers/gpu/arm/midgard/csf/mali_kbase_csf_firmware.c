...
void kbase_csf_enter_protected_mode(struct kbase_device *kbdev)
{
	...
	list_for_each_entry(kctx, &kbdev->kctx_list, kctx_list_link) {
		...
		for (r_index = 0; r_index < MAX_COHERENT_REGION; r_index++) {
			if (kctx->coherenct_regions[r_index] != NULL && (kctx->coherenct_regions[r_index])->cpu_alloc != NULL) {
				...
				for (i = 0 ; i < reg->gpu_alloc->nents; i++)
				{
					...
					dma_sync_single_for_device(kbdev->dev, sync_dma_addr, PAGE_SIZE, DMA_BIDIRECTIONAL);
				}
