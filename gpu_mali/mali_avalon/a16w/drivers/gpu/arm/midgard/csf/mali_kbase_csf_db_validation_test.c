...
static inline bool check_if_cs_ack_done(struct kbase_device *kbdev, struct kbase_csf_cmd_stream_info const *const stream, struct kbase_queue *queue, u32 group_id, u32 req_mask)
{
	bool complete = false;
	unsigned long flags;
	...
	if ((kbase_csf_fw_io_stream_read(&kbdev->csf.fw_io, group_id, stream_id, CS_ACK) & req_mask) ==
		(kbase_csf_fw_io_stream_input_read(&kbdev->csf.fw_io, group_id, stream_id, CS_REQ) & req_mask))
		complete = true;
...
static bool waiting_for_cs_ack(struct kbase_device *kbdev, struct kbase_csf_cmd_stream_info const *const stream, struct kbase_queue *queue, u32 group_id, u32 req_mask)
{
	const unsigned int fw_timeout_ms = kbase_get_timeout_ms(kbdev, CSF_FIRMWARE_TIMEOUT);
	long wt = kbase_csf_timeout_in_jiffies(fw_timeout_ms);
	long remaining;
	...
	remaining = wait_event_timeout(kbdev->csf.event_wait,
		check_if_cs_ack_done(kbdev, stream, queue, group_id, req_mask), wt);
		...
		if (kbase_csf_scheduler_group_get_slot(group) >= 0) {
			...
			for (i = 0 ; i < BASEP_GPU_QUEUE_PER_QUEUE_GROUP_MAX ; i++) {
				...
				if (queue) {
					...
					if (waiting_for_cs_ack(kbdev, stream, queue, queue->group->csg_nr, CS_REQ_EXTRACT_EVENT_MASK)) {
						res = true;
						break;
					}
				}
...
static int kbasep_csf_db_valid_standalone_test(struct seq_file *file, void *data)
{
	...
	if (!kbase_io_is_gpu_powered(entry->kbdev)) {
