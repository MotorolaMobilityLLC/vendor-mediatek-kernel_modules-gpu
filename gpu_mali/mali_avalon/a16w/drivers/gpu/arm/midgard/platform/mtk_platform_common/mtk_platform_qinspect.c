...
static int mtk_qinspect_cpuq_internal_dump(struct kbase_context *kctx, enum mtk_base_csf_notification_dump_cmd dump_cmd)
{
	...
	timeout = wait_for_completion_timeout(&kctx->csf.cpu_queue.dump_cmp, msecs_to_jiffies(300));
	...
	u32 *blocked_reason, struct mtk_qinspect_gpu_command *gpu_cmd)
{
	...
	if (queue->group->csg_nr < 0) {
		...
		*blocked_reason = queue->blocked_reason;
		...
		u64 cmd_ptr;
		struct kbase_csf_fw_io *fw_io = &queue->group->kctx->kbdev->csf.fw_io;
		...
		status_wait = (u64)kbase_csf_fw_io_stream_read(fw_io, queue->group->csg_nr, queue->csi_index, CS_STATUS_WAIT);
		*blocked_reason = (u64)kbase_csf_fw_io_stream_read(fw_io, queue->group->csg_nr, queue->csi_index, CS_STATUS_BLOCKED_REASON);
		switch (*blocked_reason) {
		case CS_STATUS_BLOCKED_REASON_REASON_SYNC_WAIT:
			gpu_cmd->gpu_sync_info.value = kbase_csf_fw_io_stream_read(fw_io, group_id, stream_id, CS_STATUS_WAIT_SYNC_VALUE);
			gpu_cmd->gpu_sync_info.cqs_addr = kbase_csf_fw_io_stream_read(fw_io, group_id, stream_id, CS_STATUS_WAIT_SYNC_POINTER_LO);
			gpu_cmd->gpu_sync_info.cqs_addr |= ((u64)kbase_csf_fw_io_stream_read(fw_io, group_id, stream_id, CS_STATUS_WAIT_SYNC_POINTER_HI)) << 32;
			...
			break;
		case CS_STATUS_BLOCKED_REASON_REASON_WAIT:
			cmd_ptr = kbase_csf_fw_io_stream_read(fw_io, group_id, stream_id, CS_STATUS_WAIT_SYNC_POINTER_LO);
			cmd_ptr |= ((u64)kbase_csf_fw_io_stream_read(fw_io, group_id, stream_id, CS_STATUS_WAIT_SYNC_POINTER_HI)) << 32;
			if (!mtk_qinspect_gpuq_internal_get_u64(queue->kctx, queue->saved_cmd_ptr, (u64 *)&inst) &&
				inst.inst.opcode == (u64)0x1f) {
				gpu_cmd->gpu_shared_sb_info.shared_entry = (u8)inst.shared_sb_dec.se;
				gpu_cmd->type = GPU_COMMAND_TYPE_SHARED_SB_WAIT;
