...
{
dev_err(kbdev->dev, "%s ++", __func__);

	if (mtk_common_gpufreq_bringup()) {
		mtk_common_pm_mfg_active();
		dev_err(kbdev->dev, "%s In if of mtk_common_gpufreq_bringup, returning 0", __func__);
		return 0;
	}
...
#endif
		dev_err(kbdev->dev, "%s In if of gpufreq_power_ctrl_enable, returning 0", __func__);
		return 0;
	}

 if (mtk_common_pm_is_mfg_active()){
	dev_err(kbdev->dev, "%s In if of mtk_common_pm_is_mfg_active, returning 0", __func__);     
	 return 0;
 }
	dev_err(kbdev->dev, "GPU PM Callback - Active");
	...
	if (gpufreq_power_control(GPU_PWR_ON) < 0) {
		...
		dev_err(kbdev->dev, "%s Power On Failed", __func__);
		return 1;
	}
...
{
	int ret = 1;
	dev_err(kbdev->dev, "%s ++, kbdev->pm.backend.gpu_powered %d", __func__, kbdev->pm.backend.gpu_powered);
WARN_ON(kbdev->pm.backend.gpu_powered);
	mutex_lock(&g_mfg_lock);
	ret = pm_callback_power_on_nolock(kbdev);
	dev_err(kbdev->dev, "%s ++, ret %d", __func__, ret);
mtk_notify_gpu_power_change(1);
#if IS_ENABLED(CONFIG_MTK_GPU_SWPM_SUPPORT)
	MTK_LTR_gpu_pmu_resume();
#endif
dev_dbg(kbdev->dev, "%s %pK\n", __func__, (void *)kbdev->dev->pm_domain);
dev_err(kbdev->dev, "%s kbdev->dev->pm_domain %pK", __func__, (void *)kbdev->dev->pm_domain);
