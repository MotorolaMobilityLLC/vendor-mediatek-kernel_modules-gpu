...
static void MTKEnableMfgClock(PVRSRV_POWER_FLAGS ePwrFlags)
{
#ifdef MTK_GPU_DVFS
...
static void MTKDisableMfgClock(PVRSRV_POWER_FLAGS ePwrFlags)
{
	int buck_state;
	...
	GED_DVFS_COMMIT_TYPE eCommitType, int *pbCommited)
{
	...
	if (psDevNode) {
		...
		if ((eResult == PVRSRV_OK) || (eResult == PVRSRV_ERROR_RETRY)) {
			...
			ui32GPUFreq =
			mt_gpufreq_get_frequency_by_level(TARGET_GPU, ui32CurFreqID);
			gpu_freq = ui32GPUFreq;
			MTKWriteBackFreqToRGX(psDevNode, ui32GPUFreq);
	#ifdef MTK_DEBUG
		if (gpu_debug_enable)
...
static IMG_UINT32 MTKCalPowerIndex(void)
{
	...
	if (PVRSRVPowerLock() != PVRSRV_OK)
		return 0;
	PVRSRVGetDevicePowerState(MTKGetRGXDevNode(), &ePowerState);
	if (ePowerState != PVRSRV_SYS_POWER_STATE_ON) {
		PVRSRVPowerUnlock();
		return 0;
	}
...
static void MTKSetBottomGPUFreq(unsigned int ui32FreqLevel)
{
	...
	gpu_bottom_freq = mt_gpufreq_get_frequency_by_level(TARGET_GPU, g_bottom_freq_id);
...
static void MTKCustomBoostGpuFreq(unsigned int ui32FreqLevel)
{
	...
	gpu_cust_boost_freq
	= mt_gpufreq_get_frequency_by_level(TARGET_GPU, g_cust_boost_freq_id);
...
static void MTKCustomUpBoundGpuFreq(unsigned int ui32FreqLevel)
{
	...
	= mt_gpufreq_get_frequency_by_level(TARGET_GPU, g_cust_upbound_freq_id);
...
#ifdef SUPPORT_PDVFS
#include "rgxpdvfs.h"
#define  mt_gpufreq_get_freq_by_idx gpufreq_get_freq_by_idx
...
static int MTKMFGOppUpdate(int ui32ThrottlePoint)
{
	...
	if (bNotReady) {
		...
		for (i = 0; i < ui32OPPTableSize; i++) {
			...
			= mt_gpufreq_get_freq_by_idx(TARGET_GPU, i) * 1000;
		}
		if (psDevNode) {
			psDevInfo = psDevNode->pvDevice;
