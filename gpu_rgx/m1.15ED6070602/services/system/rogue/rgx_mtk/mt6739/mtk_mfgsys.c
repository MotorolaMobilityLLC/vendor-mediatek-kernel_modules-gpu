...
#define mt_gpufreq_get_frequency_by_level  gpufreq_get_freq_by_idx
...
static IMG_BOOL MTKDoGpuDVFS(IMG_UINT32 ui32NewFreqID, IMG_BOOL bIdleDevice)
{
	...
	if ((eResult == PVRSRV_OK) || (eResult == PVRSRV_ERROR_RETRY)) {
		...
		//gpufreq_fix_target_oppidx(TARGET_DEFAULT, ui32NewFreqID);
		//mt_gpufreq_target(ui32NewFreqID);
		//ui32CurFreqID = __gpufreq_get_cur_idx_gpu();
		gpufreq_commit(TARGET_DEFAULT, ui32NewFreqID);
		ui32GPUFreq = gpufreq_get_freq_by_idx(TARGET_DEFAULT, ui32NewFreqID);
	...
	GED_DVFS_COMMIT_TYPE eCommitType, int *pbCommited)
{
	...
	if (psDevNode) {
		...
		if ((eResult == PVRSRV_OK) || (eResult == PVRSRV_ERROR_RETRY)) {
			...
			ui32GPUFreq =
			mt_gpufreq_get_frequency_by_level(TARGET_DEFAULT, ui32CurFreqID);
			gpu_freq = ui32GPUFreq;
...
{
	int i32MaxLevel = (int)(gpufreq_get_opp_num(TARGET_DEFAULT)-1);
	...
	gpu_freq = gpufreq_get_freq_by_idx(TARGET_DEFAULT, ui32NewFreqID);
...
{
	int i32MaxLevel = (int)(gpufreq_get_opp_num(TARGET_DEFAULT));
	int i32CurFreqID = (int)gpufreq_get_cur_oppidx(TARGET_DEFAULT);
...
#ifdef MTK_GPU_DVFS
				gpufreq_commit(TARGET_DEFAULT, g_ui32_unsync_freq_id);
#endif
				g_bUnsync = IMG_FALSE;
			}
		}
...
static void MTKSetBottomGPUFreq(unsigned int ui32FreqLevel)
{
	...
	ui32MaxLevel = gpufreq_get_opp_num(TARGET_DEFAULT);
	...
	gpu_bottom_freq = mt_gpufreq_get_frequency_by_level(TARGET_DEFAULT, g_bottom_freq_id);
	if (g_bottom_freq_id < gpufreq_get_cur_oppidx(TARGET_DEFAULT))
...
static void MTKCustomBoostGpuFreq(unsigned int ui32FreqLevel)
{
	...
	gpu_cust_boost_freq
	= mt_gpufreq_get_frequency_by_level(TARGET_DEFAULT, g_cust_boost_freq_id);
...
static void MTKCustomUpBoundGpuFreq(unsigned int ui32FreqLevel)
{
	...
	= mt_gpufreq_get_frequency_by_level(TARGET_DEFAULT, g_cust_upbound_freq_id);
...
{
	unsigned int ui32MaxLevel = gpufreq_get_opp_num(TARGET_DEFAULT);
...
#ifdef SUPPORT_PDVFS
#include "rgxpdvfs.h"
#define  gpufreq_get_freq_by_idx mt_gpufreq_get_frequency_by_level
...
PVRSRV_ERROR MTKMFGSystemInit(void)
{
	...
	g_sys_dvfs_time_ms = 0;
	g_bottom_freq_id = gpufreq_get_opp_num(TARGET_DEFAULT);
	gpu_bottom_freq = mt_gpufreq_get_frequency_by_level(TARGET_DEFAULT, g_bottom_freq_id);
	g_cust_boost_freq_id = gpufreq_get_opp_num(TARGET_DEFAULT);
	gpu_cust_boost_freq =
	mt_gpufreq_get_frequency_by_level(TARGET_DEFAULT, g_cust_boost_freq_id);
